...
public:
	...
	{
		// SO_LINGER를 사용하면 정상적 close 시에 송신되지 않은 데이터를 모두 처리한 뒤에 종료됨
		struct linger stLinger = { 0,0 };	//SO_DONTLINGER로 설정

		// bIsForce가 true이면 SO_LINGER, timeout = 0으로 설정하여 강제 종료시킴
		if (bIsForced == true)
		{
			stLinger.l_onoff = 1;
		}

		// socketClose 전에 상대방에게 전송을 중단했다
	...
	bool SendMsg(const UINT32 dataSize, char* pMsg)
	{
		...
		// 데이터가 1개이면 앞에 데이터가 없으므로 바로 wsasend
	...
	// CompletionPort 객체에 소켓과 CompletionKey를 연결시키는 함수 등
	...
	bool SendIO()
	{
		...
		// socket_error시 client socket이 끊어진 경우에 처리
		if (nRet == SOCKET_ERROR && (WSAGetLastError() != ERROR_IO_PENDING))
		{
			LOG_ERROR_ONCE("WSASend() 실패 : %d\n", WSAGetLastError());
			return false;
	...
	bool SetSockOption()
	{
		...
		{
			LOG_DEBUG("SO_RCVBUF change error: %d\n", GetLastError());
			return false;
		}
	...
	void UpdateActivity()
	{
		...
		// 클라이언트가 현재 활동 중이므로 ping 응답 기록도 리셋
...
private:
	...
	stOverlappedEx	m_stAcceptOverlappedEx;	// Accept를 요청하고 IOCP 컴플리션포트에서 완료를 확인하기 위한 구조체
	stOverlappedEx mAcceptContext;
	std::mutex mSendLock;
	bool mIsSending = false;
	UINT64 mSendPos = 0; // SendBuffer의 데이터위치 관리 변수
	...
	char mAcceptbuf[128];				// AcceptEx의 3번째 인자로 넘기는 버퍼
	UINT64 mLatestClosedTimeSec = 0;		// 마지막으로 연결이 끊어진 시각
