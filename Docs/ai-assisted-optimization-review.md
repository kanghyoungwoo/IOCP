...
IOCP 기반 채팅 서버의 브로드캐스트 경로를 최적화하는 과정에서 AI(Claude, Gemini)를 활용했습니다.
대규모 방(200~500명)에서 채팅 메시지를 브로드캐스트할 때의 성능을 개선하는 것을 목표로 삼았습니다.
...
200명 방에서 메시지를 1개 전송할 경우, **199번의 SendOverlappedEx 할당과 199번의 CopyMemory**가 발생합니다.
...
Claude에게 브로드캐스트 경로의 비효율성을 분석하도록 요청했으며, 다음 사항을 지적받았습니다:

> "200명 방에서 브로드캐스트 시, 동일한 패킷 데이터를 199번 복사하는 것은 비효율적입니다.
> 데이터를 한 번만 직렬화하고 참조 카운트로 공유하면 메모리 복사를 제거할 수 있습니다."
...
Gemini에게 동일한 코드와 Claude의 제안을 보여주고 교차 검증을 요청했습니다.
Gemini 역시 SharedSendBuffer 접근법에 동의하며, 참조 카운트 기반 공유 버퍼가 브로드캐스트 성능을 개선할 것이라고 평가했습니다.
```text
...
**50방×200명 시나리오에서 기존 코드가 개선 코드보다 압도적으로 안정적이었습니다**
...
이 398번의 atomic 연산은 1개의 캐시 라인을 8개 IOCP 워커 스레드가 동시에 경쟁합니다.
...
**절감한 복사 비용보다 추가된 atomic 경합이 20배 이상 비효율적이었습니다.**
...
- Claude와 Gemini 모두 SharedSendBuffer 도입에 동의했습니다
- 두 AI 모두 이론적 **복잡도 분석**에서는 정확했습니다 (O(N) 복사 → O(1) 참조).
- 그러나 **실제 하드웨어 수준의 비용** (캐시라인 경합, atomic 연산 지연, 커널 동작)은 고려하지 못했습니다.
- **AI의 의견이 일치하더라도 실측 검증을 대체할 수 없음을 확인했습니다**
...
기존 코드의 "각 수신자에게 독립 복사" 방식의 이점은 다음과 같습니다:
