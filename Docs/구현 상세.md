...
graph LR
    ...
    W1 & W2 & WN -->|"clientIndex push (더블버퍼, mutex)"| PP
    PP -->|"EnqueueJob (GlobalQueue SPMC)"| L1 & L2 & LN
    PP -->|Auth Request| RD
    L1 & L2 & LN -->|"Callback (MPSC Queue)"| PP
...
| **Strand/Logic** | 4 | Room별 SPSC 큐 드레인, 비즈니스 로직 직렬 처리 | `MaxLogicThread` |
...
> **튜닝 인사이트**: 채팅 빈도가 높은 고부하 환경에서는 Logic Thread에 코어를 집중(Logic=16, IO=8)하는 것이 IO Worker를 늘리는 것보다 효과적입니다. IO Worker 8→16 증가 시 CPU +2%p로 효과가 미미한 반면, Logic 8→16 증가는 Death Spiral을 완전히 해소하고 539K recv_pkt/s 안정 처리를 달성했습니다. 상세 권장값은 [부하 테스트 리포트](부하%20테스트.md)를 참조해 주십시오.
...
#### SPSC Queue — Room LocalQueue 전용 링버퍼 (Lock-Free 아님, acquire/release 메모리 오더링 기반)

```cpp
// SPSCQueue.h — alignas(64)로 False Sharing 방지
alignas(64) std::atomic<uint32_t> m_writeIndex{ 0 };  // Producer 전용 캐시라인
alignas(64) std::atomic<uint32_t> m_readIndex{ 0 };   // Consumer 전용 캐시라인
```

- **Producer(`Push`)**: relaxed load(자신 인덱스) → acquire load(상대 인덱스, full 체크) → 데이터 쓰기 → release store
- **Consumer(`Pop`)**: relaxed load(자신 인덱스) → acquire load(상대 인덱스, empty 체크) → 데이터 읽기 → release store
- **Hole 없음**: Producer가 1개이므로 exchange 경합 자체가 발생하지 않음 → `PopWithBackoff` 불필요
- **MPSCQueue와 역할 분리**: `mCallbackQueue`(Logic Workers → ProcessThread)는 실제 다중 Producer이므로 MPSCQueue 유지

[전체 구현 보기 → `SPSCQueue.h`](../IOCPChatServer/SPSCQueue.h)
...
현재 Producer는 PacketManager 단일 스레드이므로 `#define USE_SPMC`로 SPMC 경로를 활성화합니다. Producer 간 경합이 없으므로 `m_enqueuePos`의 CAS 루프가 불필요하며, relaxed load/store로 교체합니다.
...
| | SPMC (`#define USE_SPMC`) | MPMC (주석 처리) |
|---|---|---|
| `m_enqueuePos` 갱신 | `relaxed store` | `compare_exchange_weak` CAS 루프 |
| Producer 경합 | 구조적으로 없음 | CAS 루프로 해결 |
| Consumer (`Pop`) | 변경 없음 — MPMC CAS 유지 |
...
{
    mLocalQueue.Push(pJob);  // SPSCQueue<PacketJob, 4096>
...
sequenceDiagram
    ...
    W->>DB: clientIndex push (mutex)
    DB-->>PM: 버퍼 스왑 후 RingBuffer에서 패킷 조립
    PM->>SP: EnqueueJob → Room SPSC LocalQueue
    Note over PM,SP: mMsgCount==0 이면 GlobalQueue(SPMC)에 Room push
