...
> **결론: 단일 방 인원을 줄이고 방 개수를 늘리는 방향이 현 아키텍처에 적합.** 방당 1,000명 시나리오 자체에 대한 구조적 개선은 [향후 개선 방향](../README.md#향후-개선-방향)의 "브로드캐스트 구조 개선(SendToAllUser Strand 외부 비동기화)" 항목에서 추적합니다.
