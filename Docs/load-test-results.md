...
c6i.xlarge 클라이언트 4대(각 2,500봇)로 10,000명의 더미 클라이언트가 실제 유저와 동일한 시나리오(로그인 → 방 입장 → 채팅 → 방 퇴장 → 재입장)를 3~5초 간격으로 반복하는 연속 부하 테스트를 수행했습니다. 결과는 **100방 × 100명** 구성(5차) 기준입니다.
...
| **SendPool Alloc Fail** | **0 회** | 전 테스트 구간(1차~7차) 누적 |
...
본 환경에서는 단계별 부하 시험(Scenario B, 1차~7차)을 통해 다음 조합이 권장값으로 도출되었습니다.
...
| --- | --- | --- | --- | --- | --- | --- |
| 3차 | 100 × 100 | — | 23.50 ms | 1,902 ms | 127K | ~143K |
| 4차 | 100 × 100 | — | 17.93 ms | 748 ms | 140K | ~143K |
| 5차 | 100 × 100 | 33% | **17.55 ms** | **744 ms** | 134K | ~143K |
| 6차 | 200 × 50  | 50% | **20.73 ms** | 1,310 ms | 235K | **~288K** ✅ |
| 7차 | 500 × 20  | 90% | **49.24 ms** | **344 ms** | 417K | **~884K** ⚠️ |

> 3~5차는 동일 구성(100×100)에서의 반복 측정으로, 측정 환경 안정화 후 5차를 대표값으로 채택했습니다.
> 6차에서 방 개수를 2배(50→200), 방당 인원을 절반(50→1/2)으로 조정하자 **broadcast ops/s가 약 2배** 증가하면서 CPU는 50%에 머묾 → Strand 병렬도가 처리량을 결정하는 지배 요인임을 확인.
> 7차에서는 방 개수를 더 늘려(500방) p99가 오히려 344ms로 떨어지는 현상이 관찰됨. 이는 Strand 단위가 작아져 직렬화 큐가 짧게 유지된 결과.

### 부적합 영역: 방당 1,000명 (1·2차 보조 데이터)

| 차수 | 방 구성 | 환경 | CPU | Latency Avg | p99 | Recv/s |
| --- | --- | --- | --- | --- | --- | --- |
| 1차 | 1000 × 10 | c6i.xlarge | ~100% | 11,435 ms | 36,066 ms | 128K |
| 2차 | 1000 × 10 | c6i.4xlarge | ~80% | 4,202 ms | 5,741 ms | 555K |

> 1·2차 모두 평균 지연이 초 단위로 발산. 4xlarge로 스케일업해도 **방당 인원이 큰 구조에서는 단일 Strand 직렬화가 병목**이라는 사실은 변하지 않음.
