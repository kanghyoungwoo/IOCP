...
> 10,000명 동시 접속 및 초당 ~23K TPS · 44만 브로드캐스트(438K pkt/s, fan-out ×19) 환경에서 메시지 유실 제로(0 Drop) 및 평균 Latency ~9ms 달성
...
| 안정 처리량 | **~22K TPS / 415K recv_pkt/s** | 500방×20명, fan-out ×19, lat_avg 17ms |
...
| **Test** | Google Test (192 cases), Chaos Bot System, Chat Load Tester |
...
> 다만 이 리팩토링 과정에서 `IOCP.cpp`의 ZOMBIE_CLEANUP 핸들러에 남아있던 `delete` 호출을 미처 제거하지 않아, 멤버 변수 주소에 `delete`를 호출하는 힙 손상(0xC0000374) 버그가 잠재했습니다. 이는 카오스 엔지니어링 **Scenario C**(RST 강제 종료 반복)에서 발견·수정되었습니다. 단순 기능 테스트로는 재현하기 어려운 버그를 실제 공격 시나리오가 잡아낸 사례입니다.
...
graph TD
    ...
    subgraph DBLayer ["Async DB Layer"]
        Redis["Redis\n로그인 인증"]
        MySQL["MySQL\n활동 로그"]
    end
    ...
    ProcessTh <-->|"AUTH"| Redis
    ProcessTh -->|"INSERT"| MySQL
    LogicTh -->|"브로드캐스트"| IOWorkers
    LogicTh -->|"Callback (상태변경)"| ProcessTh
...
| 경로 | 흐름 |
|------|------|
| **Fast Path** (ROOM 채팅) | IO Worker → CAS 라우팅 독점 획득 → Logic Thread 직접 전달 → 브로드캐스트 |
| **Slow Path** (로그인 · 연결 · 방입장) | IO Worker → double buffering(mutex+CV) → ProcessThread → 핸들러 처리 or Logic Thread |

### 주요 구현 특징

| 기능 | 설명 |
|------|------|
| **Lock-Free MPSC/MPMC Queue** | Room LocalQueue는 MPSC CAS 기반(다중 IO Worker 직접 Push), GlobalQueue는 MPMC CAS 기반, Callback Queue는 MPSC CAS 기반 |
...
| **Deferred Send (Batch Flush)** | Logic Thread가 브로드캐스트 시 WSASend를 즉시 호출하지 않고 thread_local 리스트에 누적(EnqueueOnly) 후 Room 배치 종료 시 일괄 FlushAll — 시스템 콜 횟수 감소 |
---
...
두 아키텍처를 동일 환경에서 벤치마크하고, 측정 데이터를 기반으로 최종 채택 결정을 내렸습니다.

#### v3 — ProcessThread Double-Buffering

- **구조**: IO Worker → Double Buffering Queue(mutex) → ProcessThread(1) → Room SPSC LocalQueue → Logic Thread
- **장점**: ProcessThread가 패킷을 배치(Batch)로 모아 Room Queue에 순차 enqueue → Logic Thread의 캐시 효율 극대화. L16/IO8 구성에서 **최대 539K recv_pkt/s, lat_avg 32ms** 달성.
- **한계**: 채팅 빈도가 ProcessThread 처리 속도를 초과하면 Job Pool이 소진되며 Death Spiral 발생 — 스레드 구성과 무관한 구조적 한계.
...
> 절대적인 최대 처리량(Peak Throughput)은 v3가 미세하게 앞서지만, v3의 Death Spiral은 채팅이 몰리는 순간 서버 전체가 응답 불능 상태에 빠지며 자동 회복이 되지 않는 치명적 결함입니다.
> 6%의 처리량 차이보다 **과부하 상태에서의 장애 저항성(Resilience)** 이 상용 서버에서 훨씬 중요한 지표라고 판단하여, 최종적으로 **v4(Fast Path) 아키텍처를 채택**했습니다.
...
| 주제 | 핵심 |
|------|------|
| **스레드 모델** | IOCP Worker(8) · Logic(4) · DB(2) · ProcessThread(1) · TimeoutThread(1), 6그룹 구성 |
| **메모리 관리** | ObjectPool + LockFreeStack Free-list, Zero-Allocation 핫패스 |
| **동기화 전략** | Lock-Free 3종(MPSC/MPMC/Stack) + atomic_flag CAS(Fast Path 라우팅) + 의도적 Mutex 3곳 |
