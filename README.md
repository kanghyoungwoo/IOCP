...
수신 큐와 로직 큐를 분리하는 **더블 버퍼링(Swap)** 기법을 도입하여 락(Lock) 경합 병목을 O(1)로 단축한 결과
...
| :--- | :---: | :---: | :---: |
| **Chat RTT (평균)** | 22.0 ms | **8.0 ms** | 📉 **63% 단축** |
| **Chat RTT (p50)** | 16.8 ms | **6.7 ms** | 📉 **60% 단축** |
| **Chat RTT (p95)** | 59.1 ms | **21.1 ms** | 📉 **64% 단축** |

<br>

#### 🛡️ Phase 3: 락 기아(Starvation) 해결 및 확장성 (1,500명 접속 기준)
기존 병목으로 인해 1,000명에서 발생하던 패킷 기아 및 타임아웃 현상이 완벽하게 해결

| 지표 (Phase 3) | 단일 큐 (Before) | 더블 버퍼링 (After) | 결과 |
| :--- | :---: | :---: | :---: |
| **Login 성공** | 1,000 명 (한계) | **1,496 명** | ✅ **거의 전원 성공** |
| **Room Enter 성공** | 1,000 명 (한계) | **1,500 명** | ✅ **전원 성공** |

<br>

#### 🔥 Phase 4: 극한 부하에서의 абсолют 처리량 (2,000명 기준)
서버가 한계에 달하는 상황에서도 더 높은 처리량(Throughput)과 안정적인 응답 속도를 유지

| 지표 (Phase 4) | 단일 큐 (Before) | 더블 버퍼링 (After) | 개선율 |
| :--- | :---: | :---: | :---: |
| **초당 채팅 처리량** | 350.9 /sec | **483.3 /sec** | 📈 **37% 증가** |
| **성공한 총 채팅 수** | 123,599 건 | **158,271 건** | 📈 **28% 증가** |
| **Chat RTT (평균)** | 222.9 ms | **172.9 ms** | 📉 **22% 단축** |
